          ...
          <div className="flex gap-1" onMouseLeave={() => setHoveredStar(-1)}>
            {Array.from({ length: 5 }).map((_, i) => (
              <StarIcon
                ...
                className={`cursor-pointer w-4 aspect-square transition-all duration-300 ease-in-out ${hoveredStar !== -1 && hoveredStar < i ? 'opacity-50' : 'opacity-100'} ${i + 1 <= Math.round(product.rating.average) ? `${product.rating?.ids?.includes(user?.id ?? '') ? 'fill-yellow-600' : 'fill-yellow-500'}` : 'fill-gray-600/50'}`}
              />
            ))}
          </div>
